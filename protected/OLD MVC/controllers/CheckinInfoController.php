...
/**
	...
	public function actionViewRegCard($id){		
		...
		/*if($dashboard ==1)//crown plaza		
			...
			//$this->render('viewRegCard-leroyal',array('model'=>$model,));	
			$this->render('viewRegCard-leroyal2',array('model'=>$model,));	
...
/**
	...
	public function actionUpdate($id){
		...
		$msg = '';
		if ($guest_id==0){$msg .='Guest ID is Not Defined.. Refresh page and try again';}
		
		if(!empty($msg)){			
		$result = GuestInfo::model()->find("guest_id=:guest_id",array(":guest_id"=>100)); // 100 is random  id to run qurey	
		echo CJSON::encode( array(
		'status' => 'msg',
		'msg' => $msg,
		...
		// Uncomment the following line if AJAX validation is needed
		// $this->performAjaxValidation($model);
		if(isset($_POST['CheckinInfo']) && isset($_POST['GuestInfo'])  && $guest_id >0 ){
			$result = GuestInfo::model()->find("guest_id=:guest_id",array(":guest_id"=>$guest_id));	
	 ...
	 */	
	 public function actionCreateAjax($sroomid = ''){	
		...
		if(isset($_POST['GuestInfo']) || isset($_POST['CheckinInfo'])){ //------------
			...
			/// check guest entry in guest table.
			/* $find_guest = GuestInfo::model()->find("guest_identity_no = '". $guest_identity_no ."' || guest_mobile = '".$guest_mobile."'");			
			if(count($find_guest) > 0){ $guest_id = $model->guest_id = $find_guest->guest_id; }			
			else{	//guest_id is not set(guest==0) in checkinfo form so, guest info need to be saved in guestinfo table													
...
/**
	...
	public function actionCheckOutPopup($id=0){
		...
		if(isset($_POST['GuestLedger'])){
			...
			//if(empty($model->remarks) || $model->remarks=='')$model->remarks = '-';					
			...
			/////here we need to update the balance of already inserted payment entry rather as that balance is  creating
				 ...
				 $sql_hms_guest_ledger = "update hms_guest_ledger set balance = 0 where btc = 3 AND balance < $amount AND chkin_id = $model->chkin_id";
				  ...
				  if($model->btc==0){ $ser_acc_id= 3617; } //cash
					...
					$msg = "Dear $guest_name!... Thanks for visiting Le-Royal Guest House. Best Wishes from Le-Royal Team.";
					$msg = "We hope that u had an effortless & great stay as we tried our level best to make u feel just like at home. Thanks for staying with us. le-royal1.com";
