...
/////////////////////////
$a = explode(",",rtrim($chkstr,","));
for($i=0; $i < count($a); $i++){	
	if($i==count($a)-1)$con .= "id = ".$a[$i];
	else $con .= "id = ".$a[$i]." || ";
}
//echo "----".$chkin_id."<---". $a." --". $con;


$result = GuestLedger::model()->findAll("chkin_id=$chkin_id AND (".$con.")  order by id ASC"); 
$chkin_info = CheckinInfo::model()->findAll("chkin_id=$chkin_id");
 foreach($chkin_info as $rs){
	 ...
	 $guest_info = 	GuestInfo::model()->findAll("guest_id=$guest_id");
	 	foreach($guest_info as $r){
			...
			$company_name = Company::model()->find("comp_id = ".$r['guest_company_id'])->comp_name;		
		}
}
...
<table width="100%" border="0" align="center"> 
  ...
  <tr>
    ...
    <td><?php echo $room_name;?></td>         
    <!--  <td><?php if($dr==0)$amount = $amount; //echo $amount; ?></td>-->    
    <td><?php echo $S_name;?></td>    
    <td align="right" style="margin-right:15px;" ><?php 
	if($dr==0){ $cr =  -1 * abs($cr);  $total = $cr; }
  ...
  <tr class="tr_border">
    <td colspan="3" align="center">&nbsp;<b> Amount Payable (In Rs)</b></td>     
...
<div style="width:100%;; border-bottom: solid 1px #000; margin:5px 0;"></div>
<p align="center" style="font-size:12px">99-E Jinnah Avenue, Blue Area, Islamabad. Phone: +92-51-2277890 Fax: +92-51-2273967/2827180. Email: Info@hotelcrownplaza.com  </p>
