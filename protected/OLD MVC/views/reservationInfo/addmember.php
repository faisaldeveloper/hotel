<table class="mytbl" style="background-color:transparent;" width="200" border="0">
  <tr>
    <td width="750px"><h1> <?php echo Yii::t('views','Add New Group Member') ?> <?php //echo $model->reservation_id; ?></h1></td>    
...
if(add==1){$("#ReservationInfo_client_identity_no").inputmask("99999-9999999-9",{placeholder:"_", clearIncomplete: true });}
		...
		$('#ReservationInfo_client_name').val('');	
		$('#ReservationInfo_guest_mobile').val('');
		$('#ReservationInfo_guest_phone').val('');			
		$('#ReservationInfo_client_address').val('');	
		$('#ReservationInfo_client_country_id').val('');
		$('#ReservationInfo_client_email').val('');
		$('#ReservationInfo_client_identity_id [value="1"]').attr('selected',true);			
		$('#ReservationInfo_client_identity_no').val('');	
});
...
-->
 ...
 function totalres_ob(){ 
  ...
  <span class="mcetr" <?php echo $sh; ?> id="group_row2">    	  
    ...
    <?php echo $form->textField($model,'total_reservations',array('value'=>1, 'size'=>30,'maxlength'=>30, 'onBlur'=>'totalres_ob()')) ?>
 ...
 <div class="row" style="float:left; margin-left:5px;">	  
	  ...
	  <?php echo $form->hiddenField($model,'reservation_id',array('size'=>'5', 'Maxlength'=>'15', 'readonly'=>'readonly','value'=>$model->reservation_id)); ?>
		<?php //echo $form->error($model,'total_days'); ?>        
 </div>
 ...
 <div class="row" style="float:left; margin-left:5px;">         
  ...
  <legend class="mcelegend" style="color:#900;">Guest Person Information:</legend>
   ...
   <?php echo $form->labelEx($model,'client_name'); ?>
	...
	echo $form->dropDownList($model,'client_salutation_id', CHtml::listData(Salutation::model()->findAll(), 'salutation_id','salutation_name')); 
	echo "&nbsp;&nbsp".$form->textField($model,'client_name',array('size'=>30,'maxlength'=>30, 'autocomplete'=>'off',  'onkeyup'=>'suggest(this.value)' ));
	?>
...
<fieldset>
  <legend class="mcelegend" style="color:#900;">Room Info</legend>
   ...
   <?php
	...
	echo $form->dropDownList($model,'room_type', CHtml::listData(HmsRoomType::model()->findAll("branch_id=$bid"), 'room_type_id', 'room_name'),array('onchange' => CHtml::ajax(
						array(
							'type' => 'POST',
...
function suggest(inputString){
	...
	function fill(id, name, mobile, phone, address, country, email, identity_id, identity_no) { 	
		...
		$('#ReservationInfo_guest_mobile').val(mobile);
		$('#ReservationInfo_guest_phone').val(phone);			
		$('#ReservationInfo_client_address').val(address);	
		$('#ReservationInfo_client_country_id').val(country);
		$('#ReservationInfo_client_email').val(email);
		$('#ReservationInfo_client_identity_id [value="'+ identity_id+'"]').attr('selected',true);			
		$('#ReservationInfo_client_identity_no').val(identity_no);		
